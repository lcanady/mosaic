...
export const setAttr = async (
  { target, name, value, setBy, tags }: AttributeOptions,
) => {
  ...
  await dbobjs.updateOne({ dbref: setBy }, {
    $set: { "data.attributes": attr },
  });
};

export const getAttr =  (target: DbObj, name: string) => {
  return target.data.attributes?.find( a => a.name === name.toLowerCase() )?.value 
};
...
export const delAttr = async (target: string, name: string) => {
  ...
  if (index > -1) {
    attr.splice(index, 1);
    await dbobjs.updateOne({ dbref: target }, {
      $set: { "data.attributes": attr },
    });
  }
};
